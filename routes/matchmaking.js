...
app.get("/fortnite/api/game/v2/matchmakingservice/ticket/player/*", verifyToken, (req, res) => {
    ...
    const config = global.lawinConfig || JSON.parse(fs.readFileSync("./Config/config.json").toString());
    const useWss = config.useWss || process.env.RAILWAY_PUBLIC_DOMAIN;

    res.json({
        "serviceUrl": `${useWss ? "wss" : "ws"}://${config.matchmakerIP}`,
